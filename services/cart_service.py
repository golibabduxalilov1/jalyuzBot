"""
Ready sizes cart service.

MUHIM:
- Mavjud admin panel, sotuvchilar, hamkorlar va boshqa logikalarga tegilmaydi.
- Faqat tayyor razmerlar (sheets6) uchun karzinka funksiyasi.
"""
...
# RAM ichida saqlanadigan karzinka:
# key: cart_id (har bir qo'shilgan element alohida)
_READY_SIZES_CART: Dict[str, CartItem] = {}

# 24 soatlik TTL
_CART_TTL = timedelta(hours=24)


def _make_cart_key(user_id: int, code: str, razmer: str) -> str:
    return f"{user_id}:{normalize_code(code)}:{(razmer or '').strip()}"


def _generate_cart_id() -> str:
    """Har bir karzinka elementi uchun qisqa unique ID yaratadi."""
    while True:
        cart_id = uuid4().hex[:16]
        if cart_id not in _READY_SIZES_CART:
            return cart_id


def _cleanup_expired_items() -> None:
    """24 soatlik muddatdan oshgan karzinka elementlarini avtomatik tozalash."""
    if not _READY_SIZES_CART:
        return
    now = datetime.utcnow()
    keys_to_delete: List[str] = []
    for key, item in _READY_SIZES_CART.items():
        if now - item.added_at > _CART_TTL:
            keys_to_delete.append(key)
    for key in keys_to_delete:
        _READY_SIZES_CART.pop(key, None)
...
def get_or_create_cart_item(
    ...
    """
    Karzinkaga bitta mahsulotni qo'shish.
    Har bir qo'shilgan mahsulot alohida element sifatida saqlanadi.
    ...
    _READY_SIZES_CART[cart_id] = item
    
    # LOG CART EVENT: Item added to cart
    from services.ready_sizes_events import log_cart_event
    try:
        log_cart_event(
            model_nomi=model_nomi,
            code=code_val,
...
def remove_cart_item(
    ...
    """Karzinkadan bitta elementni olib tashlash."""
    item = None
    key_to_remove = None

    if cart_id:
        item = _READY_SIZES_CART.get(cart_id)
        if item:
            key_to_remove = cart_id
    else:
        code_norm = normalize_code(code)
        razmer_norm = normalize_razmer(razmer or "")
        for key, existing_item in _READY_SIZES_CART.items():
    ...
    # LOG DELETED EVENT: Item removed from cart
    if item:
    ...
    if key_to_remove:
        _READY_SIZES_CART.pop(key_to_remove, None)


def get_cart_item_by_id(cart_id: str) -> Optional[CartItem]:
    """cart_id bo'yicha karzinka elementini qaytaradi."""
    _cleanup_expired_items()
    if not cart_id:
        return None
    return _READY_SIZES_CART.get(cart_id)


def get_cart_items_for_admin_view() -> List[CartItem]:
    """
    Admin panel → Tayyor razmerlar → Karzinka uchun barcha elementlarni qaytarish.
    Har chaqirilganda 24 soatlik muddat tugaganlarini avtomatik tozalaydi.
    """
    _cleanup_expired_items()
    return list(_READY_SIZES_CART.values())


def get_cart_items_for_user(user_id: int) -> List[CartItem]:
    """
    Faqat o'sha foydalanuvchining karzinkasidagi elementlarni qaytarish.
    Har chaqirilganda 24 soatlik muddat tugaganlarini avtomatik tozalaydi.
    """
    _cleanup_expired_items()
    user_items = []
    for item in _READY_SIZES_CART.values():
        if item.user_id == user_id:
            user_items.append(item)
    return user_items


def is_in_cart_any_user(code: str, razmer: str) -> bool:
    """
    Berilgan code+razmer hozirda istalgan foydalanuvchi karzinkasida bormi yoki yo'qmi.
    Tayyor razmerlar natijalari ostida status ko'rsatish uchun ishlatiladi.
    """
    _cleanup_expired_items()
    code_norm = normalize_code(code)
    razmer_clean = (razmer or "").strip()
    for item in _READY_SIZES_CART.values():
        if (
            normalize_code(item.code) == code_norm
            and (item.razmer or "").strip() == razmer_clean
        ):
            return True
    return False


def get_cart_quantity_by_code(code: str, razmer: str) -> int:
    """
    Berilgan code+razmer bo'yicha karzinkada nechta olinganini qaytaradi.
    Barcha foydalanuvchilarning karzinkalarini hisoblaydi.
    ...
    Returns:
        Karzinkaga olingan donalar soni (0 yoki ko'p)
    """
    _cleanup_expired_items()
    code_norm = normalize_code(code)
    razmer_norm = normalize_razmer(razmer or "")
...
def build_cart_owner_text(
    item: CartItem,
) -> str:
    """
    Karzinka elementi qaysi foydalanuvchi va qaysi sotuvchi/hamkor bilan bog'liqligini text ko'rinishida qaytarish.
    ...
    if is_admin(item.user_id) and not is_super_admin(item.user_id):
        ...
        if admin_name and not admin_name.startswith("User"):
            lines.append(f"👤 Karzinkaga olgan: {admin_name} (ID: {item.user_id})")
        else:
            lines.append(f"👤 Karzinkaga olgan: {item.user_name} (ID: {item.user_id})")
    ...
    # Agar sotuvchi bo'lsa
        ...
        lines.append(f"👤 Karzinkaga olgan: {seller_display} (ID: {item.user_id})")
        lines.append("🏷 Rol: Sotuvchi")
    # Agar hamkor bo'lsa
    elif item.is_partner:
        user_display = item.user_name
        lines.append(f"👤 Karzinkaga olgan: {user_display} (ID: {item.user_id})")
