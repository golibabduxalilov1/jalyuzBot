...
async def _edit_menu_message(
    bot: Bot, chat_id: int, new_text: str, new_keyboard: InlineKeyboardMarkup
):
    """
    ...
    try:
        ...
        await bot.edit_message_text(
            chat_id=chat_id, message_id=msg_id, text=new_text, reply_markup=new_keyboard
        )
        return True
...
def make_back_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="menu_astatka")],
...
def make_astatka_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="📊 Umumiy qoldiqdan bilish", callback_data="astatka_general"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [InlineKeyboardButton(text="⬅️ Orqaga", callback_data="astatka_back_main")],
        ]
    )
...
def make_result_keyboard_general() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⬅️ Orqaga", callback_data="astatka_general")]
        ]
    )
...
async def callback_astatka_menu(
    ...
    try:
        ...
        menu_msg = await bot.send_message(
            chat_id=chat_id, text=menu_text, reply_markup=keyboard
        )
        _store_menu_message(chat_id, menu_msg)


@router.callback_query(F.data == "astatka_general")
async def callback_astatka_general(
    callback_query: CallbackQuery, state: FSMContext, bot: Bot
):
    ...
    menu_text = "📊 <b>Umumiy qoldiqdan bilish</b>\n\n" "Mahsulot kodini yuboring:"
    ...
    if is_result_page:
        ...
        try:
            ...
            if (
                chat_id in _RESULT_MESSAGES
                and _RESULT_MESSAGES[chat_id] == callback_query.message.message_id
            ):
...
async def process_product_code(message: Message, state: FSMContext, bot: Bot):
    ...
    if not increment_user_request_count(user_id):
        await message.answer(
            "❌ Kunlik so'rovlar limitiga yetdingiz. Ertaga qayta urinib ko'ring."
        )
        return
    ...
    if not user_code:
        ...
        error_msg = await bot.send_message(
            chat_id, "❌ Iltimos, to'g'ri mahsulot kodini yuboring."
        )
    ...
    try:
        ...
        result_keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⬅️ Orqaga", callback_data="astatka_general"
                    ),
                    InlineKeyboardButton(
        ...
        if not product_data:
            ...
            error_msg = await bot.send_message(
                chat_id, error_text, reply_markup=result_keyboard
            )
        ...
        try:
            ...
            for price in prices:
                ...
                price_code_normalized = (
                    normalize_code(str(price_code)) if price_code else ""
                )
        ...
        is_rollo_shtor = (
            "ROLLO SHTOR" in model_name_upper
            or "rollo shtor" in model_name_lower
            or "ролло штор" in model_name_lower
            or ("ROLLO" in model_name_upper and "SHTOR" in model_name_upper)
        )

        is_dikey = (
            "ДИКИЙ" in model_name_upper
            or "Дикий" in model_name_original
            or "дикий" in model_name_lower
            or "ДИКЕЙ" in model_name_upper
            or "ДИКЕЙ" in model_name_original
            or "dikey" in model_name_lower
            or "дикей" in model_name_lower
            or "дикей" in model_name_original
        )

        is_plise = (
            "PLISE" in model_name_upper
            or "plise" in model_name_lower
            or "плисе" in model_name_lower
            or "плисе" in model_name_original
        )
        ...
        for row in matched_rows:
            ...
            if (
                "asosiy" in row_collection.lower()
                or "основной" in row_collection.lower()
            ):
                ...
                "kasetniy" in row_collection.lower()
                or "кассетный" in row_collection.lower()
                or "kaset" in row_collection.lower()
            ):
        ...
        original_code = (
            product_data.get("original_code") or product_data.get("code") or user_code
        )
        ...
        # Дикий uchun maxsus format
            ...
            if asosiy_qoldiq:
                lines_list = [
                    f"• To'ldi uzi bo'lsa: {qty_str}" for _, qty_str in asosiy_qoldiq
                ]
                header += "\n".join(lines_list) + "\n"
            if mini_qoldiq:
                lines_list = [
                    f"• Yoniga porter bo'lsa: {qty_str}" for _, qty_str in mini_qoldiq
                ]
                header += "\n".join(lines_list) + "\n"
        ...
        if has_image:
            ...
            try:
                        ...
                        if isinstance(image_url, bytes):
                            ...
                            try:
                                await bot.delete_message(
                                    chat_id, previous_result_msg_id
                                )
                            except Exception:
                                pass
                            ...
                            if image_file_id:
                                media = InputMediaPhoto(
                                    media=image_file_id, caption=text
                                )
                            else:
                            ...
                            try:
                                ...
                                await state.set_state(
                                    AstatkaStates.waiting_for_code_general
                                )
                                return
                            except Exception as edit_error:
                                logger.warning(
                                    f"Error editing photo message: {edit_error}, trying to delete and send new"
                                )
                ...
                logger.error(
                    f"Error sending photo: {photo_error}, sending text instead"
                )
                ...
                result_msg = await bot.send_message(
                    chat_id, text, reply_markup=result_keyboard
                )
        else:
        ...
        result_msg = await bot.send_message(
            chat_id, f"❌ Xatolik yuz berdi: {str(e)}\n\nIltimos, qayta urinib ko'ring."
        )
        _store_result_message(chat_id, result_msg)
        await state.set_state(AstatkaStates.waiting_for_code_general)
...
def make_collection_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="0-start", callback_data="collection_0-start")],
            [InlineKeyboardButton(text="1-stage", callback_data="collection_1-stage")],
            [
                InlineKeyboardButton(
                    text="2-middle", callback_data="collection_2-middle"
                )
            ],
            [
                InlineKeyboardButton(
                    text="3-optimal", callback_data="collection_3-optimal"
                )
            ],
            [InlineKeyboardButton(text="4-top", callback_data="collection_4-top")],
            [
                InlineKeyboardButton(
                    text="5-perfect", callback_data="collection_5-perfect"
                )
            ],
            [
                InlineKeyboardButton(
                    text="6-exclusive", callback_data="collection_6-exclusive"
                )
            ],
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    text="Ролло штор", callback_data="collection_Ролло штор"
                )
            ],
            [InlineKeyboardButton(text="Дикей", callback_data="collection_Дикей")],
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="menu_astatka")],
        ]
    )
...
async def callback_astatka_collection(
    callback_query: CallbackQuery, state: FSMContext, bot: Bot
):
    ...
    menu_text = "📁 <b>Kolleksiya bo'yicha bilish</b>\n\n" "Kolleksiyani tanlang:"
...
async def callback_collection_selected(
    callback_query: CallbackQuery, state: FSMContext, bot: Bot
):
    ...
    if is_user_blocked(user_id):
        await callback_query.answer(
            "❌ Siz bloklangan foydalanuvchisiz.", show_alert=True
        )
        return
    ...
    try:
        ...
        result_keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⬅️ Orqaga", callback_data="astatka_collection_back"
                    ),
                    InlineKeyboardButton(
                        text="🏠 Asosiy menyu", callback_data="menu_main"
                    ),
                ...
                try:
                    if has_photo and target_edit_id == current_msg_id:
                        ...
                        try:
                            ...
                            result_msg = await bot.send_message(
                                chat_id, error_result_text, reply_markup=result_keyboard
                            )
                            store_content_message(chat_id, result_msg.message_id)
                            _store_result_message(chat_id, result_msg)
                        except Exception:
                        ...
                        if target_edit_id == current_msg_id:
                            ...
                            _store_result_message(
                                chat_id, _create_msg_obj(target_edit_id)
                            )
                except Exception as e:
        ...
        if (
            not edit_success
            and previous_result_msg_id
            and previous_result_msg_id != current_msg_id
        ):
...
async def callback_astatka_similar_codes(
    callback_query: CallbackQuery, state: FSMContext, bot: Bot
):
    ...
    menu_text = "🔍 <b>O'xshash kodlar bo'yicha qoldiqni bilish</b>\n\n" "Kod yuboring:"
        ...
        try:
            if has_photo:
                ...
                try:
                    ...
                    if (
                        chat_id in _RESULT_MESSAGES
                        and _RESULT_MESSAGES[chat_id] == current_msg_id
                    ):
                        _RESULT_MESSAGES.pop(chat_id, None)
...
async def on_astatka_similar_codes_back(
    ...
    try:
        ...
        result_keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⬅️ Orqaga", callback_data="astatka_similar_codes_back"
                    ),
                    InlineKeyboardButton(
                        text="🏠 Asosiy menyu", callback_data="menu_main"
                    ),
...
async def on_astatka_collection_back(
    callback_query: CallbackQuery, bot: Bot, state: FSMContext
):
    """
...
async def on_astatka_back_main(
    callback_query: CallbackQuery, bot: Bot, state: FSMContext
):
    """
